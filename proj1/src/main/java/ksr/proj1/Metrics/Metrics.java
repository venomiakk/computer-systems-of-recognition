...
public class Metrics {
    ...
    float euclideanDistance(FeatureVector featureVector1, FeatureVector featureVector2, WordsSimilirityMetrics metric) {
        float distance = 0.0f;
    ...
    float manhattanDistance(FeatureVector featureVector1, FeatureVector featureVector2, WordsSimilirityMetrics metric) {
        float distance = 0.0f;
        for(int i=0; i< featureVector1.features.size();i++){
            if (featureVector1.features.get(i).getClass() == featureVector2.features.get(i).getClass()) {
                if (featureVector1.features.get(i) instanceof String) {
    ...
    public float chebyshevDistance(FeatureVector featureVector1, FeatureVector featureVector2, WordsSimilirityMetrics metric){
        ...
        return distance.stream().max(Float::compare).get();
    }
